...
interface ServiceCardsProps {
  /** Render inside country hero (no section heading, compact layout) */
  embedded?: boolean;
}

export const ServiceCards: React.FC<ServiceCardsProps> = ({ embedded = false }) => {
  ...
  const cards = (
    <ul className={`mx-auto grid w-full gap-4 ${embedded ? 'max-w-2xl' : 'max-w-2xl'}`}>
      {destination.services.map((service) => (
        <li
          key={service.slug}
          className={`overflow-hidden rounded-4xl shadow-lg ${
            embedded
              ? 'border border-white/20 bg-white'
              : 'border border-blue-100/60 bg-gradient-to-br from-white via-blue-50/40 to-blue-100/30 shadow-sm'
          }`}
        >
  ...
  if (embedded) {
    return <div id="services">{cards}</div>;
  }
