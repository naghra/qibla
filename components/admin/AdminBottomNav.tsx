        ...
        {items.map(({ to, end, label, icon: Icon, badge }) => (
          <li key={to} className="flex-1">
            <NavLink
              ...
              className={({ isActive }) =>
                `relative flex flex-col items-center gap-0.5 px-2 py-2.5 text-[10px] font-medium transition sm:text-xs ${
            ...
            >
              ...
              <span>{label}</span>
              {badge && pending > 0 && (
                <span className="absolute end-1/4 top-1 flex size-4 items-center justify-center rounded-full bg-amber-500 text-[9px] font-bold text-white">
                  {pending > 9 ? '9+' : pending}
                </span>
              )}
            </NavLink>
